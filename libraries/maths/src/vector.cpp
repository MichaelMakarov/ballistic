...
const char* vc_same_size{ "ожидались векторы одинаковых размерностей" };

vector& vector::operator+=(const vector& other) {
	ASSERT(_size == other._size, vc_same_size);
...
double operator* (const vector& left, const vector& right) {
	ASSERT(left._size == right._size, vc_same_size);
...
const char* empty_vector{ "вектор пустой" };

double& vector::first() {
	ASSERT(_size > 0, empty_vector);
	return _elems[0];
}
