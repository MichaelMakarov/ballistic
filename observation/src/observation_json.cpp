...
namespace {
    ...
    void serialize(archive &ar, observation &o) {
        ar &YAS_OBJECT_NVP("observation", ("ID", o.id), ("norad", o.norad), ("kiam", o.kiam), ("nko", o.nko), ("seances", o.seances));
    }

    template <typename archive>
    void serialize(archive &ar, observatory &o) {
        ar &YAS_OBJECT_NVP("observatory",
                           ("ID", o.id),
                           ("latitude", o.lat),
                           ("longitude", o.lon),
                           ("altitude", o.alt),
    ...
    void serialize(archive &ar, observatories_group &g) {
        ar &YAS_OBJECT_NVP("group", ("observatories", g.observatories));
    }
    ...
    std::vector<observatory> read_observatories_from_json(std::string_view filename) {
        return deserialize_from_json<observatories_group>(filename).observatories;
    }

    observation read_observation_from_json(std::string_view filename) {
        return deserialize_from_json<observation>(filename);
    }
...
std::vector<observation_seance> load_sparkle_observation_from_json(std::string_view obs_filename, std::string_view mes_filename) {
    ...
    for (const auto &s : observation.seances) {
        ...
        auto observ = std::find_if(std::begin(observatories), std::end(observatories), [&s](const observatory &obs) {
            return s.observatory == obs.id;
        });
        if (observ == std::end(observatories))
            continue;
        observation_seance seance{.id = static_cast<unsigned>(std::stoul(observ->id)), .o = {observ->x, observ->y, observ->z}};
        std::set<measurement_data> ranged_obs;
        for (const auto &m : s.track) {
            if (m[3] == 0 || m[2] == 0 || m[1] == 0)
                continue;
            ranged_obs.insert(measurement_data{
    ...
    std::sort(std::begin(seances), std::end(seances), [](observation_seance const &left, observation_seance const &right) {
