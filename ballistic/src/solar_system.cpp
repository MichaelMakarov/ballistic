...
namespace ball {
	...
	RIA solar_model::position(const times::juliandate& jd) 
	{
		...
		const double jc = jd_to_jc2000(jd);	// julian centures since 2000
	...
	RIA lunar_model::position(const times::juliandate& jd) 
	{
		...
		return ECS_to_ACS(sph_to_ort(moon), ecl);
	}
	...
	XYZ solar_position(const JD& jd)
	{
		...
		return ACS_to_GCS(sph_to_ort(solar_model::position(jd)), sidt);
	}

	XYZ lunar_position(const JD& jd)
	{
		double sidt = sidereal_time_mean(jd);
		return ACS_to_GCS(sph_to_ort(lunar_model::position(jd)), sidt);
	}
